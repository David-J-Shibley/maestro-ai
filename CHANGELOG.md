...
[1.9.4]: https://github.com/David-J-Shibley/maestro-ai/releases/tag/v1.9.4
...
[1.9.3]: https://github.com/David-J-Shibley/maestro-ai/releases/tag/v1.9.3

## [1.9.2] - 2026-08-10

### Added
- **Model catalog validation** — `probe`, `maestro doctor`, and proxy startup verify each configured `model` id appears in the gateway’s `/v1/models` list (catches Maestro config ↔ LiteLLM/Ollama mismatches like `qwen3-4b` missing from `model_list`).
- Exported `validateConfiguredModels`, `isModelInCatalog`, `fetchModelCatalog` from the package API.

### Fixed
- **`maestro doctor` litellm_process** — no longer treats `vi litellm.yaml` as a running LiteLLM server.

[1.9.2]: https://github.com/David-J-Shibley/maestro-ai/releases/tag/v1.9.2
...
### Added
- **CLI/MCP test & build hooks** — `--run-tests` / `--run-build` (and MCP `run_tests` / `run_build`) wire shell commands into the evaluator’s `tests_pass` / `build_pass` checks so implement-test-fix workflows can actually recover.
- **Cost truth** — per-attempt usage/cost on retries and escalations; telemetry `totalEstimatedCostUsd`; session spend prefers the summed total; budget re-checked mid-escalation with in-flight costs.
- **Structured feedback** — `rating` (1–5) and `accepted` on `maestro feedback`, MCP `maestro_feedback`, and `POST /v1/feedback`; patched onto telemetry for analyze/learned.
- **Savings vs always-premium** — `maestro stats` reports counterfactual premium cost, estimated savings, and savings rate; optional acceptance rate / avg rating when feedback exists.
- Analyze/learned blend acceptance and ratings into tier recommendations.
- **Offline → local-only** — when the machine has no internet (or probe shows hosted + premium down while local is up), Maestro caps routing at `local_strong`. Config: `routing.offlineLocalOnly` (default `true`). Surfaced on `GET /status` as `connectivity`.

[1.7.0]: https://github.com/David-J-Shibley/maestro-ai/releases/tag/v1.7.0
...
### Fixed
- **Fail-soft routing** — premium / tool-forwarding require strong evidence (imperative + target, high risk, architecture). Short pings like `testing` / `hi are you working` no longer escalate to Bedrock via keyword collisions (`test` ⊂ `testing`) or harness tool catalogs.
- Word-boundary keyword matching; broader soft chitchat / status-ping detection.
- Tool-need score no longer clears the threshold from task-type alone; tool-use floor stays `local_strong` unless evidence is strong.

[1.5.1]: https://github.com/David-J-Shibley/maestro-ai/releases/tag/v1.5.1
